...
b = BPF(src_file="host_agent.ebpf.c")
...
def print_event(cpu, data, size):
    ...
    if(re.match("10.50.1.*", res_ip)):
        print(f"INIT FID -> SADDR:{res_ip}, PORT:{data.init_port}, PROTO:{data.init_proto}, SZ:{data.init_sz}, T:{unsignedToSigned(data.init_tm, 8)}", end=" ")
    else:
        print(f"INIT FID -> SADDR:{ip_xor}, PORT:{data.init_port}, PROTO:{data.init_proto}, SZ:{data.init_sz}, T:{unsignedToSigned(data.init_tm, 8)}", end=" ")

    print(f"RESULT -> DADDR:{socket.inet_ntoa(data.res_addr.to_bytes(4, 'little'))}, DPORT:{data.res_port}, PROTO:{data.res_proto}, SZ:{data.res_sz}, T:{unsignedToSigned(data.res_tm, 8)}")
